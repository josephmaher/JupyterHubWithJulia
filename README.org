...
* Linode (or any Ubuntu) server setup
...
To change the base_url (needed for proxying through another website) run this as root inside the container:
...
The changes should show up in ~/opt/tljh/config/config.yaml~

The apache forwarding settings that seem to work are here, assuming the remote machine is running on port 8001:

#+begin_src
Define IP <whatever the ip of the machine running the container is>

RewriteEngine on

RewriteCond %{HTTP:Connection} Upgrade [NC]
RewriteCond %{HTTP:Upgrade} websocket [NC]
RewriteRule /juliabox/(.*) ws://${IP}:8001/juliabox/$1 [P,L]
RewriteRule /juliabox/(.*) http://${IP}:8001/juliabox/$1 [P,L]

<Location /juliabox> 
 
ProxyPreserveHost On
ProxyPass http://${IP}:8001/juliabox/
ProxyPassReverse http://${IP}:8001/juliabox/
