...
#+begin_src
export JULIA_DEPOT_PATH="/opt/julia/1.12.5:"
julia -e 'print(DEPOT_PATH)'
julia -e 'using Pkg; Pkg.update(); Pkg.add("MYPACKAGE"); Pkg.precompile()'
#+end_src

Replace 1.12.5 by the version of julia you are using.  Check that the /opt/julia/1.12.5
path comes first in the list of paths.
...
You will need to run the following command on the wireguard server (currently proxy)

#+begin_src
sudo wg set wg0 peer $PUBLIC_KEY allowed-ips $LOCAL_IP/32
#+end_src

And then start wireguard in the container.
